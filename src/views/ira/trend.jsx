...
const IraTrend = () => {
    ...
    const meanData = useSelector((state) => state.ira.iraMean)
    const lowerData = useSelector((state) => state.ira.iraLower)
    const upperData = useSelector((state) => state.ira.iraUpper)

    const ira_2016 = useSelector((state) => state.ira.ira_2016)
    const ira_2017 = useSelector((state) => state.ira.ira_2017)
    const ira_2018 = useSelector((state) => state.ira.ira_2018)

    const mean_2016 = useSelector((state) => state.ira.mean_2016)
    const lower_2016 = useSelector((state) => state.ira.lower_2016)
    const upper_2016 = useSelector((state) => state.ira.upper_2016)
    ...
    useEffect(() => {
        ...
        if (!upperData)
            dispatch(fetchIraUpper({ params: params.upper, engine }))
        if (!ira_2016)
            dispatch(fetchIra2016({ params: params['2016'], engine }))
        if (!ira_2017)
            dispatch(fetchIra2017({ params: params['2017'], engine }))
        if (!ira_2018)
            dispatch(fetchIra2018({ params: params['2018'], engine }))
        if (!mean_2016)
    ...
    useEffect(() => {
        if (
            ...
            dispatch(setIraDataTable(combinedData))
        }
    ...
    useEffect(() => {
        if (activeOrgUnit) {
            setYearData({
                2021: combineValuesByOrgUnits(activeOrgUnit, ira_2016),
                2022: combineValuesByOrgUnits(activeOrgUnit, ira_2017),
                2023: combineValuesByOrgUnits(activeOrgUnit, ira_2018),
            })
            setMinMaxData({
            ...
            datasets: [
                {
                    ...
                    data:
                        yearData[2021] ||
                        (ira_2016 ? addValues(orgUnits, ira_2016) : []),
                ...
                },
                {
                    ...
                    data:
                        yearData[2022] ||
                        (ira_2017 ? addValues(orgUnits, ira_2017) : []),
                ...
                },
                {
                    ...
                    data:
                        yearData[2023] ||
                        (ira_2018 ? addValues(orgUnits, ira_2018) : []),
